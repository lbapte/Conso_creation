...
const SegmentationPage = () => {
  ...
  const [currentFilterIndex, setCurrentFilterIndex] = useState<number | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    const fetchOptions = async () => {
      try {
        ...
        console.error(
          "Erreur lors de la récupération des options de circuit et période :",
          error
        );
      }
    };
  ...
  useEffect(() => {
    setActiveFilterCount(
      selectedFilters.filter((f) => f !== "Aucun filtre").length
    );
  ...
  const fetchReferenceItems = async (
    ...
    if (!dynamicData[compositeKey]) {
      try {
        ...
        console.error(
          "Erreur lors de la récupération des références (3 filtres) :",
          error
        );
      }
    }
  ...
  const fetchReferencesForOneFilter = async (
    ...
    if (!dynamicData[key]) {
      try {
        ...
        console.error(
          "Erreur lors de la récupération des références (1 filtre) :",
          error
        );
      }
    }
  ...
  const fetchReferencesForTwoFilters = async (
    ...
    if (!dynamicData[key]) {
      try {
        ...
        console.error(
          "Erreur lors de la récupération des références (2 filtres) :",
          error
        );
      }
    }
  ...
  useEffect(() => {
    const updateData = async () => {
      const data = await fetchUniqueValuesBySegmentation(
        selectedFilters[0],
        advancedFilter
      );
  ...
  const applyAdvancedFilter = () => {
    ...
    const effectiveColumn =
      advancedIndicateur !== "" ? advancedIndicateur : advancedSegmentation;
    ...
    if (advancedSegmentation !== "") {
      ...
      const loadAutoComplete = async () => {
        try {
          ...
          console.error(
            "Erreur lors du fetchUniqueValues pour autocomplétion :",
            error
          );
          setAutoCompleteOptions([]);
        }
        ...
        <View style={styles.fullPage}>
          <SafeAreaView style={styles.containerTransp}>
            <View style={styles.topHeader}>
              ...
              <View style={styles.filterAdvancedActionsContainer}></View>
            </View>
            ...
            <View style={styles.filterActionsContainer}>
              ...
              >
                ...
                <Text
                  style={styles.filterViaSelected}
                  numberOfLines={1}
                  ellipsizeMode="tail"
                >
                  {filtrerVia}
            ...
            >
              <Text style={styles.advancedToggleText}>
                {showAdvancedFilter
                  ? "Masquer le filtre avancé"
                  : "Afficher le filtre avancé"}
              </Text>
            </TouchableOpacity>
                ...
                <View style={styles.selectionButtonsContainer}>
                  <TouchableOpacity
                    style={[
                      ...
                      advancedIndicateur !== "" && styles.selectedButton,
                    ]}
                    onPress={() => {
                  ...
                  <TouchableOpacity
                    style={[
                      styles.selectionButton,
                      advancedSegmentation !== "" && styles.selectedButton,
                    ]}
                    onPress={() => {
                    ...
                    <View>
                      ...
                      >
                        <View style={styles.chevronContainer}>
                          <FontAwesome
                            name={
                              expandedItems[key] ? "chevron-up" : "chevron-down"
                            }
                            size={14}
                            color="#2B26BF"
                          />
                              ...
                              return (
                                <View>
                                  <TouchableOpacity
                                    style={styles.subListItem}
                                    onPress={() =>
                                      toggleExpandSecond(firstItem, secondItem)
                                    }
                                  >
                                    <View style={styles.chevronContainer}>
                                      <FontAwesome
                                  ...
                                  {expandedItems[compositeKey2] &&
                                    ...
                                    dynamicData[compositeKey2].length > 0 && (
                                      <FlatList
                                        ...
                                        keyExtractor={(item, index) =>
                                          `level3-${firstItem}-${secondItem}-${index}`
                                        }
            ...
            >
              ...
              <View style={styles.modalContainer}>
                <View style={styles.modalContent}>
                  ...
                  <TouchableOpacity
                    style={styles.closeButton}
                    onPress={() => setFiltrerViaModalVisible(false)}
                  >
                    <Text style={styles.closeButtonText}>Fermer</Text>
                  </TouchableOpacity>
                </View>
            ...
            >
              <View style={styles.modalContainer}>
                <View style={styles.modalContent}>
                  ...
                  <TouchableOpacity
                    style={styles.closeButton}
                    onPress={() => setCircuitModalVisible(false)}
                  >
                    <Text style={styles.closeButtonText}>Fermer</Text>
                  </TouchableOpacity>
                </View>
            ...
            >
              {selectedEan && (
                <ModalPage barcode={selectedEan} onClose={closeRefModal} />
              )}
            </Modal>
          </SafeAreaView>
        </View>
